...
function checkDogs(dogsJulia, dogsKate) {
    //1
    let dummyArr = dogsJulia.slice(1, 3);
    console.log(dummyArr);
    console.log(dogsJulia);

    //2
    let combinedArr = dummyArr.concat(dogsKate);
    console.log(combinedArr);

    //3
    combinedArr.forEach((age, i) => {
        let val = age >=5 ? `Dog number ${i+1} is an adult, and is ${age} years old` : `Dog number ${i+1} is still a puppy`;
        console.log(val);
...
const calcAverageHumanAge = function(ages) {
    ...
    return humanAge/humanAge.length;
}
